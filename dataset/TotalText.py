...
class TotalText(TextDataset):
    def __init__(
        ...
        self.annotation_list = [
            "poly_gt_{}".format(img_name.replace(".jpg", ""))
    ...
    def parse_carve_txt(gt_path):
        ...
        for line in lines:
            ...
            try:
                xx = [int(x) for x in re.split(" *", xx)]
                yy = [int(y) for y in re.split(" *", yy)]
            except:
                xx = [int(x) for x in re.split(" +", xx)]
                yy = [int(y) for y in re.split(" +", yy)]
            if len(xx) < 4 or len(yy) < 4:
                continue
            text = gt[-1].split("'")[1]
            try:
                ori = gt[-2].split("'")[1]
...
if __name__ == "__main__":
    ...
    for idx in range(0, len(trainset)):
        ...
        direction_map = cav.heatmap(
            np.array(
                direction_field[0] * 255 / np.max(direction_field[0]), dtype=np.uint8
            )
        )
