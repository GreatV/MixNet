...
class ArtTextJson_mid(TextDataset):
    def __init__(
        self,
        data_root,
        is_training=True,
        ...
        self.image_root = os.path.join(
            data_root, "train_images" if is_training else "test_images"
        )
        self.image_list = os.listdir(self.image_root)
        self.image_list = list(
            filter(
                lambda img: img.replace(".jpg", "") not in ignore_list, self.image_list
            )
        )
        if self.is_training:
            annotation_file = os.path.join(
                data_root, "train_labels.json" if is_training else "None"
            )
            self.annotation_data = load_json(annotation_file)
            self.image_list, self.annotationdata_list = self.preprocess(
                self.image_list, self.annotation_data
            )
        if self.load_memory:
            self.datas = list()
...
if __name__ == "__main__":
    ...
    trainset = ArtTextJson(
        data_root="/home/prir1005/pubdata/ArT", is_training=True, transform=transform
    )
    t0 = time.time()
    (
        img,
    ...
    distance_map = cav.heatmap(
        np.array(distance_field * 255 / np.max(distance_field), dtype=np.uint8)
    )
    cv2.imshow("distance_map", distance_map)
    cv2.waitKey(0)
    direction_map = cav.heatmap(
        np.array(direction_field[0] * 255 / np.max(direction_field[0]), dtype=np.uint8)
    )
    ...
    weight_map = cav.heatmap(
        np.array(weight_matrix * 255 / np.max(weight_matrix), dtype=np.uint8)
    )
