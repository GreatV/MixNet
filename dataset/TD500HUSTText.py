...
class TD500HUSTText(TextDataset):
    def __init__(
        self,
        data_root,
        is_training=True,
        ...
        if is_training:
            ...
            self.image_list = self.image_list + sorted(
                [
                    os.path.join(data_root, fname)
                    for fname in fnames
                    if img_check.findall(fname)
                ]
            )
            self.anno_list = self.anno_list + sorted(
                [
                    os.path.join(data_root, fname)
                    for fname in fnames
                    if gt_check.findall(fname)
                ]
            )
...
if __name__ == "__main__":
    ...
    trainset = TD500HUSTText(data_root="./data/", is_training=True, transform=transform)
