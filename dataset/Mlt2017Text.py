    ...
    def __init__(
        ...
        if is_training:
            ...
            if ignore_list:
                ...
                ignore_list = []
            self.img_list = self.img_val_list + self.img_train_list
        else:
...
if __name__ == "__main__":
    ...
    trainset = Mlt2017Text(
        data_root="../data/MLT2017", is_training=True, transform=transform
    )
    for idx in range(0, len(trainset)):
        ...
        cv2.imshow(
            "tr_mask",
            cav.heatmap(
                np.array(train_mask * 255 / np.max(train_mask), dtype=np.uint8)
            ),
        )
        cv2.imshow("imgs", img)
        cv2.waitKey(0)
