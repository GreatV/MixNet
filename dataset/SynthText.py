...
if __name__ == "__main__":
    ...
    transform = Augmentation(size=512, mean=means, std=stds)
    trainset = SynthText(
        data_root="../FAST/data/SynthText", is_training=True, transform=transform
    )
    for idx in range(0, len(trainset)):
        ...
        print(idx, tuple(img.shape), tuple(tr_mask.shape))
        tr_mask = np.array(tr_mask * 255 / np.max(tr_mask), dtype=np.uint8)
        print(tuple(tr_mask.shape))
