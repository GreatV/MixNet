...
class TextDataset(object):
    ...
    def generate_proposal_point(
        ...
        if jitter > 0:
            x_offset = (
                (np.random.rand(tuple(ctrl_points.shape)[0]) - 0.5) * distance * jitter
            )
            y_offset = (
                (np.random.rand(tuple(ctrl_points.shape)[0]) - 0.5) * distance * jitter
            )
    ...
    def compute_direction_field(inst_mask, h, w):
        ...
        nearPixel[0, :, :] = x
        nearPixel[1, :, :] = y
        grid = np.indices(tuple(inst_mask.shape))
    ...
    def check_poly(self, polys):
        if tuple(polys.shape)[0] == 4:
            if polys[0][0] - polys[1][0] + (polys[0][1] - polys[1][1]) < polys[1][
                0
            ] - polys[2][0] + (polys[1][1] - polys[2][1]):
                polys = polys[::-1]
        return polys

    def get_gt_midline(self, polys):
        half = tuple(polys.shape)[0] // 2
        topline = polys[:half]
    ...
    def make_text_region(self, img, polygons):
        h, w = tuple(img.shape)[0], tuple(img.shape)[1]
        mask_zeros = np.zeros(tuple(img.shape)[:2], np.uint8)
        train_mask = np.ones((h, w), np.float)
        ...
        gt_mid_points = np.zeros(
            (cfg.max_annotation, cfg.num_points // 2, 2), dtype=np.float
        )
        ...
        for idx, polygon in enumerate(polygons):
            ...
            dmp = ndimg.distance_transform_edt(inst_mask)
            maxdmp = np.max(dmp)
            if (
                polygon.text == "#"
                or maxdmp < self.min_text_size
    ...
    def get_training_data(self, image, polygons, image_id=None, image_path=None):
        ...
        image = paddle.to_tensor(data=image).astype(dtype="float32")
        train_mask = paddle.to_tensor(data=train_mask).astype(dtype="float32")
        tr_mask = paddle.to_tensor(data=tr_mask).astype(dtype="int32")
        weight_matrix = paddle.to_tensor(data=weight_matrix).astype(dtype="float32")
        direction_field = paddle.to_tensor(data=direction_field).astype(dtype="float32")
        distance_field = paddle.to_tensor(data=distance_field).astype(dtype="float32")
        gt_points = paddle.to_tensor(data=gt_points).astype(dtype="float32")
        proposal_points = paddle.to_tensor(data=proposal_points).astype(dtype="float32")
        ignore_tags = paddle.to_tensor(data=ignore_tags).astype(dtype="int32")
        gt_mid_points = paddle.to_tensor(data=gt_mid_points).astype(dtype="float32")
        edge_field = paddle.to_tensor(data=edge_field).astype(dtype="int32")
        return (
            image,
            train_mask,
            tr_mask,
    ...
    def get_test_data(self, image, polygons=None, image_id=None, image_path=None):
        ...
        if polygons is not None:
            for i, polygon in enumerate(polygons):
                pts = polygon.points
                points[i, : tuple(pts.shape)[0]] = polygon.points
                length[i] = tuple(pts.shape)[0]
                if polygon.text != "#":
                    label_tag[i] = 1
                else:
                    label_tag[i] = -1
