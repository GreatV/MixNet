...
class TD500HUSTText_mid(TextDataset):
    ...
    def __init__(self, data_root, is_training=True, ignore_list=None,
        load_memory=False, transform=None):
        ...
        if is_training:
            ...
            self.image_list = self.image_list + sorted([os.path.join(
                data_root, fname) for fname in fnames if img_check.findall(
                fname)])
            self.anno_list = self.anno_list + sorted([os.path.join(
                data_root, fname) for fname in fnames if gt_check.findall(
                fname)])
...
if __name__ == '__main__':
    ...
    trainset = TD500HUSTText(data_root='./data/', is_training=True,
        transform=transform)
    for idx in range(0, len(trainset)):
        ...
        img, train_mask, tr_mask = map(lambda x: x.cpu().numpy(), (img,
            train_mask, tr_mask))
        ...
        print(tuple(gt.shape))
        print(tuple(poly.shape))
        for i in range(len(poly)):
            for j in range(20):
                cv2.circle(img, tuple(gt[i, j]), 3, (255, 0, 0), -1)
                cv2.circle(img, tuple(poly[i, j]), 3, (0, 255, 0), -1)
        cv2.imwrite('tr_mask.jpg', np.array((tr_mask > 0) * 255, dtype=np.
            uint8))
        cv2.imwrite('imgsshow.jpg', img)
        distance_field = image_info[3].numpy()
        distance_map = cav.heatmap(np.array(distance_field * 255 / np.max(
            distance_field), dtype=np.uint8))
