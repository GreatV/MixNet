...
class RandomErasing(object):
    ...
    def __call__(self, img, polygons=None):
        if random.random() < self.ratio:
            return img, polygons
        area = tuple(img.shape)[0] * tuple(img.shape)[1]
        target_area = random.randint(*self.sr) * area
        aspect_ratio = random.uniform(*self.scale)
        h = int(round(math.sqrt(target_area / aspect_ratio)))
        w = int(round(math.sqrt(target_area * aspect_ratio)))
        if w < tuple(img.shape)[1] and h < tuple(img.shape)[0]:
            x1 = random.randint(0, tuple(img.shape)[1] - w)
            y1 = random.randint(0, tuple(img.shape)[0] - h)
...
class RotatePadding(object):
    ...
    def __call__(self, image, polygons=None, scale=1.0):
        ...
        if do_90 < 0.1:
            ...
            angle = angle - 90
        rows, cols = tuple(image.shape)[0:2]
        center = cols / 2.0, rows / 2.0
...
class RandomCropFlip(object):
    ...
    def __call__(self, image, polygons=None):
        ...
        if len(polygons_new) == 0:
            ...
            cropped = image[ymin:ymax, xmin:xmax, :]
            height, width, _ = tuple(cropped.shape)
            select_type = random.randint(3)
            if select_type == 0:
                img = np.ascontiguousarray(cropped[:, ::-1])
                for polygon in polygons_new:
...
class ResizeSquare(object):
    ...
    def __call__(self, image, polygons=None):
        ...
        if img_size_min < self.size[0]:
            im_scale = float(self.size[0]) / float(img_size_min)
            if np.ceil(im_scale * img_size_max) > self.size[1]:
...
class RandomResizePadding(object):
    def __init__(
        ...
        step_num = round(np.random.normal(loc=0.0, scale=0.35) * 8)
...
class BaseTransform(object):
    def __init__(self, size, mean, std):
        ...
        self.std = std
        self.augmentation = Compose(
            [ResizeSquare(size=self.size), Normalize(mean, std)]
        )
