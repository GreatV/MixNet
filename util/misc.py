...
def to_device(*tensors):
    if len(tensors) < 2:
        return tensors[0].to(cfg.device, blocking=not True)
    return (t.to(cfg.device, blocking=not True) for t in tensors)
...
def rescale_result(image, bbox_contours, H, W):
    ori_H, ori_W = tuple(image.shape)[:2]
...
def draw_gaussian(heatmap, center, radius, k=1, delte=6):
    ...
    x, y = center
    height, width = tuple(heatmap.shape)[0:2]
...
def get_cosine_map(embed, threshold=0.85, imgfname=None):
    step = paddle.concat(x=(embed[:, :, 1:, :], embed[:, :, -1:, :]), axis=2)
    step = paddle.concat(x=(step[:, :, :, 1:], step[:, :, :, -1:]), axis=3)
    assert tuple(step.shape) == tuple(embed.shape)
    cosine_map = paddle.nn.functional.cosine_similarity(
        x1=embed, x2=step, axis=1, eps=1e-08
    )
    if imgfname is not None:
        cosine_map = (cosine_map - 1) * -0.5
        cosine_map = paddle.nn.functional.max_pool2d(
            x=cosine_map, kernel_size=2, stride=1
        )
        cosine_map = cosine_map / paddle.max(x=cosine_map)
        cosine_map = paddle.clip(x=cosine_map, min=0, max=1)
        vis_map = cosine_map[0].detach().cpu().numpy() * 255
        vis_map = vis_map.astype(np.int32)
        cv2.imwrite("vis/cosine_map/" + imgfname, vis_map)
        return cosine_map
    return paddle.clip(
        x=(cosine_map + 1) / (2 * paddle.max(x=cosine_map)), min=0, max=1
    )
